...
const Row = ({ isButtonSchedule, isModificate=false, data }) => {
    ...
    const [paramsCancelAppointment , setParamsCancelAppointment] = useState(null);  

    //const params= `?page=${pageNumber}`
	const token = sessionStorage.getItem('token')
    const urlPostScheduleAppointment = endpoints.postScheduleAppointment //+ params
    const { data:dataPostScheduleAppointment, refetch: refetchPostScheduleAppointment } = useQuery({
		...
		queryFn: ()=> postFetch(urlPostScheduleAppointment, entriesData, token),
		enabled: false,
	}) 

    const url = endpoints.putCancelAppointment + paramsCancelAppointment
    const { data: dataPutCancelAppointment, refetch: refetchPutCancelAppointment } = useQuery({
        queryKey: ["key-putCancelAppointment"],
        queryFn: ()=> putFetch(url, null, token),
        enabled: false,
    }) 
console.log(data)
    ...
    const handleClickCancelAppointment = () => {
        setParamsCancelAppointment(data.id)
       
    }

    useEffect(()=>{

        if (paramsCancelAppointment) {
            const confirm = window.confirm('Realmente quiere cancelar este turno?')
            if(confirm) {
                refetchPutCancelAppointment()
                    .then(()=>console.log('listo'))
            }
        }

    },[paramsCancelAppointment])
    ...
    return (
        <div>
            <div className={`grid grid-cols-4 border border-[#5666BF] rounded-3xl ${modificate ? 'rounded-b-none' : ''} transition-all duration-300 overflow-hidden`}>
                <div className={`flex justify-center items-center gap-5 text-center p-6 transition-colors duration-300 ${isButtonSchedule ? '' : modificate ? '' : 'bg-[#f0efef]'}`}>
                    ...
                    <div className="flex flex-col justify-center items-center gap-5">
                        {modificate
                            ? <input type="date" className="w-fit py-2" />
                            : <div className="flex flex-col justify-center items-center gap-5">
            ...
            <div className={`${modificate ? "transform translate-y-0" : "transform -translate-y-full"}  flex justify-center items-center text-white font-bold bg-[#12b00f] p-2 rounded-bl-full duration-300 flex-1 cursor-pointer`}>
                ...
                <div 
                    className={`${modificate ? "transform translate-y-0" : "transform -translate-y-full"}  flex justify-center items-center text-white font-bold bg-[#b00f0f] p-2 rounded-br-full duration-300 flex-1 cursor-pointer`}
